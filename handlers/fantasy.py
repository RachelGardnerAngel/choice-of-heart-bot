...
from keyboards.reply_kb import get_fantasy_keyboard, get_back_only_keyboard
...
async def setting_get_male_1(message: Message):
    await send_setting(message, DESC_GET_MALE_1, "get_male")

@router.message(F.text == SETTING_GET_MALE_2)
async def setting_get_male_2(message: Message):
    await send_setting(message, DESC_GET_MALE_2, "get_male")

@router.message(F.text == SETTING_GET_MALE_3)
async def setting_get_male_3(message: Message):
    await send_setting(message, DESC_GET_MALE_3, "get_male")

@router.message(F.text == SETTING_GET_MALE_4)
async def setting_get_male_4(message: Message):
    await send_setting(message, DESC_GET_MALE_4, "get_male")

@router.message(F.text == SETTING_GET_MALE_5)
async def setting_get_male_5(message: Message):
    await send_setting(message, DESC_GET_MALE_5, "get_male")

@router.message(F.text == SETTING_GET_MALE_6)
async def setting_get_male_6(message: Message):
    await send_setting(message, DESC_GET_MALE_6, "get_male")

# ========== ОБЩАЯ ФУНКЦИЯ ==========
async def send_setting(message: Message, description: str, category: str):
    user_id = message.from_user.id
    logger.info(f"Пользователь {user_id} открыл сеттинг в {category}")
